...
--joinCheck(mId,sId)
SELECT * FROM SSTATUS WHERE MID = 'test1-1' AND SID = 2 AND SSSTATUS = 1;
--joinStudy(sId,mId)
INSERT INTO SSTATUS (SSID, MID, SID, SSSTATUS)
            VALUES (SSTATUS_SEQ.NEXTVAL, 'teemo',1,1);
--unJoin(sId,mId)
DELETE FROM SSTATUS WHERE MID='test1-1' AND SID='2' AND SSSTATUS = 1;
SELECT * FROM SSTATUS;
SELECT COUNT(*) FROM SSTATUS WHERE MID = 'test1-1' AND SID = 2 AND SSSTATUS = 1;
--joinList(sId)
SELECT MIMAGE, M.MID, SID, SSSTATUS,MNICKNAME FROM MEMBER M, SSTATUS S WHERE M.MID = S.MID AND S.SID = 2 AND SSSTATUS= 1;
--acceptStudy(mId)
