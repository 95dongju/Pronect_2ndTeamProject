...
-- DROP TABLE
DROP TABLE GSTATUS;
DROP TABLE GROUP_BOARD_REPLY; -- 이름 변경 -> GROUP_BOARD_COMMENT
...
DROP SEQUENCE GROUP_BOARD_REP_SEQ; -- 이름 변경 -> GROUP_BOARD_CMT_SEQ
DROP SEQUENCE GROUP_BOARD_CMT_SEQ;
...
CREATE TABLE GROUP_BOARD_COMMENT (
    GROUP_CMTID NUMBER(5) PRIMARY KEY,
    GROUP_BID NUMBER(5) NOT NULL,
    MID VARCHAR2(100) NOT NULL,
    GROUP_BOARD_CMT_CONTENT VARCHAR2(1000) NOT NULL,
    GROUP_BOARD_CMT_LIKE NUMBER(5) DEFAULT 0,
    GROUP_BOARD_CMT_RDATE DATE DEFAULT SYSDATE,
    FOREIGN KEY (GROUP_BID) REFERENCES GROUP_BOARD (GROUP_BID),
    FOREIGN KEY (MID) REFERENCES MEMBER (MID)
);

-- SEQUENCE
CREATE SEQUENCE GROUP_BOARD_CMT_SEQ INCREMENT BY 1 START WITH 11111 MINVALUE 11111 MAXVALUE 99999 NOCYCLE CACHE 50000 ORDER;
----------------------------------------------------------------------
--------------------------  GROUP STATUS  ----------------------------
----------------------------------------------------------------------
...
ALTER TABLE FBOARD DROP COLUMN FLIKE;