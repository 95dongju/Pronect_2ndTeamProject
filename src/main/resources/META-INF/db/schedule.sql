...
-- updateAchive
UPDATE SCD_ACHIVE SET SCD_ACHIVE = 'Y' WHERE SCD_ID = 11131 AND MID = 'test1-1';

-- 개인 참여율 확인
SELECT CASE 
    WHEN NVL((SELECT COUNT(*) FROM SCHEDULE WHERE GID = 1), 0) > 0 
    AND NVL((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 13  AND S.MID = 'test1-1'), 0) > 0 
    
    THEN ROUND((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 13 AND S.MID = 'test1-1')
...
SELECT CASE 
    WHEN NVL((SELECT COUNT(*) FROM SCHEDULE WHERE GID = 1), 0) > 0 
    AND NVL((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 13), 0) > 0 
    
    THEN ROUND((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 13)
    / (SELECT COUNT(*) FROM SCHEDULE WHERE GID = 1) * 100) ELSE 0 END "rate" FROM DUAL;
