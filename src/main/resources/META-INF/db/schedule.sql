...
-- updateAchive
UPDATE SCD_ACHIVE SET SCD_ACHIVE = 'Y';
commit;
-- 개인 참여율 확인
SELECT CASE 
    WHEN NVL((SELECT COUNT(*) FROM SCHEDULE WHERE GID = 15), 0) > 0 
    AND NVL((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 15 AND S.MID = 'test4-2'), 0) > 0 
    
    THEN ROUND((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 15 AND S.MID = 'test4-2')
    / (SELECT COUNT(*) FROM SCHEDULE WHERE GID = 15) * 100) ELSE 0 END "rate" FROM DUAL;

SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 9 AND S.MID = 'test1-1';
...
SELECT CASE 
    WHEN NVL((SELECT COUNT(*) FROM SCHEDULE WHERE GID = 15), 0) > 0 
    AND NVL((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 15), 0) > 0 
    
    THEN ROUND((SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE A.MID = S.MID AND A.SCD_ID = S.SCD_ID AND SCD_ACHIVE = 'Y' AND S.GID = 15)
    / (SELECT COUNT(*) FROM SCD_ACHIVE A, SCHEDULE S WHERE S.SCD_ID = A.SCD_ID AND GID = 15) * 100) ELSE 0 END "rate" FROM DUAL;
